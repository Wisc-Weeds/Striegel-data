...
data1 <- data %>% 
  ...
  unite("site_year", c("site", "year"), remove = FALSE) %>% 
...
```{r}
leveneTest(biored ~ year, data = data1)
```
...
```{r}
data2 <- data1 %>% 
  nest(site_year, year, siteyr, block, trt, trtdes, post, residual, biored)
```
...
data7 %>% 
  group_by(site, post) %>% 
#  mutate(herb = reorder_within(herb, by = response, within = rate)) %>% 
