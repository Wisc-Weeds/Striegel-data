...
#HORIZONTAL
ggplot(sum, aes(x = factor(gly, level=level_order), y=Mean)) + 
  geom_hline(yintercept=5, linetype="dashed", color = "#646569", lwd=1) +
  geom_pointrange(aes(y =Mean, ymin  =  Trad.lower, ymax  =  Trad.upper, color=gly), size  =  0.7) +
  facet_grid(SA~.) + 
  scale_color_manual(values=c( "#0479a8" ,"#C5050C", "#adadad", "#282728"), name="", 
                    labels=c("Durango DMA", "No glyphosate","Premix", "Roundup PowerMax")) +
  theme_bw() + theme(#axis.title.y = element_text(size=25, color="black"),
                     ...
                     strip.text.y = element_text(size=15, angle=360, hjust=0, color="black", face="bold"),
  ...
  labs(y="pH", x="") + coord_flip() + scale_y_continuous(breaks=c(4.5, 5.0, 5.5, 6.0, 6.5, 7.0, 7.5)) +
  ggsave("pH+glyV2.4-2.pdf", units="in", width=7, height=5, dpi=600)
...
ggplot(sum, aes(x = factor(gly, level=level_order), y=Mean)) + 
  geom_pointrange(aes(y =Mean, ymin  =  Trad.lower, ymax  =  Trad.upper, color=gly), size  =  0.7) +
  facet_grid(.~SA) + 
  scale_color_manual(values=c( "#646569" ,"#C5050C", "#adadad", "#282728"), name="", 
                    labels=c("Durango DMA", "No glyphosate","Premix", "Roundup PowerMax")) +
  theme_bw() + theme(axis.title.y = element_text(size=15, color="black"),
                     ...
                     legend.title = element_blank()) +
  labs(y="pH", x="") +  scale_y_continuous(breaks=c(4.5, 5.0, 5.5, 6.0, 6.5, 7.0, 7.5)) +
  ggsave("pH+gly2.tiff", units="in", width=7, height=5, dpi=600)
```
...
rf_grid <- grid_regular(mtry(range=c(2,5)), min_n(range=c(1,5)), levels=5)
set.seed(456)
...
final_rf %>%
  ...
  theme(plot.title = element_text(hjust=0.5, size=35, face="bold"),
                     ...
                     panel.grid.minor=element_line(color="white")) +
  labs(y="Variable importance") +
  ggsave("pHs2RFv2.1.tiff", units="in", width=6, height=6, dpi=600)
...
final_rf %>%
  ...
  fit(ph ~ ., data=s2_train) %>%
  #vi() %>%
  vip(geom="col", horizontal=TRUE, aesthetics=list(fill=c("#A50F15", "#CB181D", "#EF3B2C"), 
                                                   width= 0.65)) +
  scale_fill_brewer(palette="Reds") +
  theme_light() + 
  theme(plot.title = element_text(hjust=0.5, size=35, face="bold"),
                     ...
                     panel.grid.minor=element_line(color="white")) +
  labs(y="Variable importance") +
  ggsave("pHs2RFv2.2.tiff", units="in", width=6, height=6, dpi=600)
...
rf_grid <- grid_regular(mtry(range=c(1,5)), min_n(range=c(1,6)), levels=5)
set.seed(456)
...
final_rf %>%
  ...
  theme(plot.title = element_text(hjust=0.5, size=35, face="bold"),
                     ...
                     panel.grid.minor=element_line(color="white")) +
  labs(y="Variable importance") +
  ggsave("pHs3RFv2.1.tiff", units="in", width=6, height=6, dpi=600)
...
final_rf %>%
  ...
  fit(ph ~ ., data=s3_train) %>%
  #vi() %>%
  vip(geom="col", horizontal=TRUE, aesthetics=list(fill=c("#A50F15", "#CB181D", "#EF3B2C", "#FB6A4A"), 
                                                   width= 0.65)) +
  scale_fill_brewer(palette="Reds") +
  theme_light() + 
  theme(plot.title = element_text(hjust=0.5, size=35, face="bold"),
                     ...
                     panel.grid.minor=element_line(color="white")) +
  labs(y="Variable importance") +
  ggsave("pHs3RFv2.2.tiff", units="in", width=6, height=6, dpi=600)
...
dat4=read.csv("study 4 RF v2.csv",header=TRUE, na.strings="")
set.seed(123)
...
rf_grid <- grid_regular(mtry(range=c(1,4)), min_n(range=c(15,30)), levels=5)
set.seed(456)
...
final_rf %>%
  ...
  theme(plot.title = element_text(hjust=0.5, size=35, face="bold"),
                     ...
                     panel.grid.minor=element_line(color="white")) +
  labs(y="Variable importance") +
  ggsave("pHs4RFv2.1.tiff", units="in", width=6, height=6, dpi=600)
...
final_rf %>%
  ...
  theme(plot.title = element_text(hjust=0.5, size=35, face="bold"),
                     ...
                     panel.grid.minor=element_line(color="white")) +
  labs(y="Variable importance") +
  ggsave("pHs4RFv2.2.tiff", units="in", width=6, height=6, dpi=600)
